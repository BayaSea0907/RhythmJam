...
public class Music : MonoBehaviour
{
	...
	public class Section
	{
        ...
        //これはCreateSectionClips == trueの場合にのみ機能します。
        // this will only work when CreateSectionClips == true.
        public ClipType LoopType;

		public Section(int startBar, int mtBeat = 4, int mtBar = 16, double _bpm = 120)
		{
			StartBar = startBar;
			UnitPerBeat = mtBeat;
			UnitPerBar = mtBar;
		...
		{
			return string.Format("\"{0}\" StartBar:{1}, bpm:{2}", Name, StartBar, bpm);
		}
	}
    ...
    // 追記:     2017/11/21
    //  Beatの半分取得用
    public static bool IsJustChangedHalfBeat()
    {
        return Current_.IsJustChangedHalfBeat_();
    }

    //----------------------------------------------------------
    // 追記:     2018/01/08
    //  Unitもとる
    public static bool IsJustChangedUnit()
    {
        return Current_.IsJustChangedUnit_();
    }
    //----------------------------------------------------------
    // 追記:     2018/01/08
    //  Unitの半分
    public static bool IsJustChangedHalfUnit()
    {
        return Current_.IsJustChangedHalfUnit_();
    }
    ...
    // 追記:     2017/11/21
    //  Beatの半分取得用
    public static bool IsNearChangedHalfBeat()
    {
        return Current_.IsNearChangedHalfBeat_();
    }
    ...
    //----------------------------------------------------------
    // TODO: sectionIdxは消す。foreaceで名前から取得できるようにする。
    //
    public static void Play(string audioName, int sectionIndex)
    {
        MusicList_.Find((Music m) => m.name == audioName).PlayStart(Current_.Sections[sectionIndex].Name);
    }
    /// <summary>
    ...
    private bool IsNearChangedHalfBeat_()
    {
        return isJustChanged_ && just_.HalfBeat == 1;
    }
    ...
    // 追記:  2018/01/08
    //  Unitも実装
    private bool IsJustChangedUnit_()
    {
        return isJustChanged_;
    }
    private bool IsJustChangedHalfUnit_()
    {
        return just_.HalfUnit == 1;
    }
    ...
    //  Beatのさらに半分のタイミングを取得
    //
    private bool IsJustChangedHalfBeat_()
    {
        return isJustChanged_ && just_.Unit % 2 == 0 && just_.HalfBeat == 1;
    }
	...
	void Awake()
	{
		...
		if( musicSource_.loop )
		{
			...
			double beatSec = (60.0 / lastSection.bpm);
    ...
    // スクリプトがロードされたとき、またはインスペクタで値が変更されたときに呼び出されます
    void OnValidate()
	{
		...
		else
		{
			...
			for( int i = 0; i < Sections.Count; i++ )
			{
				...
				if( i+1 < Sections.Count )
				{
					...
					timeSamples += (int)((d / Sections[i].UnitPerBeat) * (60.0f / Sections[i].bpm) * samplingRate_);
				}
			}
		}
    ...
    void OnSectionChanged()
	{
		...
		if( CurrentSection_.bpm > 0.0f )
		{
			double beatSec = (60.0 / CurrentSection_.bpm);
	...
	void UpdateTiming()
	{
        ...
        if (sectionSample >= 0)
        {
            ...
            // 追記:  2017/11/21 
            //  Beatの半分
            just_.HalfBeat = (int)(((sectionSample % samplesPerBar_) % samplesPerBeat_) / samplesPerUnit_ / 2);
            ...
            //  Unitの半分更新
            /*
            if (isFormerHalf_ == false && !just_.halfUnitFlg)
            {
                just_.HalfUnit = 1;
                just_.halfUnitFlg = true;
            }
            else if(isFormerHalf_ == false)
            {
                just_.HalfUnit = 0;
            }
            else
	...
	void CallEvents()
	{
        ...
        if (isJustChanged_ && just_.Unit == 0 && just_.HalfBeat == 0) OnHalfBeat();
...
{
	public Timing(int bar = 0, int beat = 0, int unit = 0, int halfBeat = 0)
	{
		Bar = bar;
		Beat = beat;
		Unit = unit;
	...
	public void Init() { Bar = 0; Beat = 0; Unit = 0; HalfUnit = 0; HalfBeat = 0; }
	public void Copy(Timing copy)
	{
		Bar = copy.Bar;
		Beat = copy.Beat;
		Unit = copy.Unit;
        HalfBeat = copy.HalfBeat;

    }
	public void Set(int bar, int beat = 0, int unit = 0, int halfBeat = 0)
	{
		Bar = bar;
		Beat = beat;
		Unit = unit;
    ...
    // -------------------------------------------------------
    // 追記:  2017/11/21
    //  HalfBeat...Beatよりさらに細かい部分の取得用
    //
    public string tag;
    public int Bar, Beat, HalfBeat, Unit, HalfUnit;
    [HideInInspector] public bool halfUnitFlg = false;
	...
	public void Fix(Music.Section section)
	{
        ...
        // HalfUnit = (totalUnit - Bar * section.UnitPerBar - Beat * section.UnitPerBeat);
        // HalfUnit = totalUnit;// (totalUnit - Bar * section.UnitPerBar - Beat * section.UnitPerBeat);
    }
	...
	{
		return (this.Bar == other.Bar && this.Beat == other.Beat && this.Unit == other.Unit && this.HalfBeat == other.HalfBeat);
	}
	...
	public override string ToString()
	{
        return "Bar=" + Bar + " Beat=" + Beat + " Unit=" + Unit + " HalfBeat= " + HalfBeat + " HalfUnit= " + HalfUnit + "  ";        
	}
}